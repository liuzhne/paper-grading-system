...
拒绝继续提高 HTTP 超时、由浏览器保持长连接、由 Vercel Cron 执行整批评分、一次请求拆成多次但仍由浏览器串行触发，以及把 HTTP 200 当作评分完成。初始材料并发为 2，并依据 provider 配额与耗时观测调整。代价是增加常驻 worker 与调度运维；收益是刷新或断网不影响执行，平台终止后能按租约从检查点恢复。数据库已有 job/item 表，按用户授权不新增无实际结构变化的 Alembic 迁移。
