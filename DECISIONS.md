...
| D-032 | Accepted | 未配置模型用阻断式弹窗，而不是跳转加横幅 |
| D-033 | Accepted | 复核表的行内「确认」只给可采纳行，阻塞行不摆假入口 |
| D-034 | Accepted | 逐项确认与批量采纳共用一条提交路径 |
| D-035 | Accepted | AI 起草的落库走 `recompile`，前端做合并 |
| D-036 | Accepted | 删掉 `StageStub` 与指向旧版界面的文案 |
| D-037 | Accepted | 篇章与格式发现接进工作区，复用已到手的 run 对象 |
| D-038 | Accepted | `.btn` 的居中修在类本身；文件框改样式而不换控件 |
| D-039 | Accepted | 弹窗按「这一次导航被拦了」显示，不按会话级开关 |
| D-040 | Accepted | 能力表在任何改变可用性的写操作后立即刷新，双向 |
...
- 实施补充（2026-09-09）：范围必须**先于** `publish_rubric()` 写入。有 ORM 守卫拦住「已发布
  评分标准不可修改」，发布后再改会直接抛错。两者仍在同一个事务里，要么一起生效要么一起回滚——
  分两次提交会留下「已发布但范围还是旧的」这个**没有补救入口**的中间态，因为发布后范围就冻结了。
...
### D-032：未配置模型用阻断式弹窗，而不是跳转加横幅

- 状态：`Accepted`；2026-09-09（用户决定）。
- 背景：初版是路由守卫把用户跳到账户页，再在页面上显示一条 `notice` 横幅。
- 选择：改为阻断式弹窗——先说清「本部署尚未配置可用模型」，再由用户点进对应配置页
  （平台管理员两条出路，普通用户一条）。判定留在 `requiresModelSetup`，呈现交给组件。
- 原因：横幅容易被忽略；而且跳转把用户送到一个他没主动去的页面，得自己猜发生了什么。
- 放弃：跳转加横幅（初版）。
- 代价/约束：弹窗覆盖全站，因此**不能遮挡配置动作**——进了运维页仍要能填表。
  能力表未加载时不弹，避免首屏误报。
- 修正（2026-09-10，用户反馈）：初版设计成**不可关闭**，理由是「没有可用模型时整套
  能力都用不了，给关闭等于放人进去撞失败」。这条站不住——遮罩 `inset: 0` 铺满视口，
  用户点进配置页之后弹窗还在，**表单点不到**，把人挡在了他正要去做的那件事前面。
  改为可关闭：补 ✕，点操作链接也自动收起。
  同时暴露一个**假验收**：原用例断言 `toBeVisible()`，而遮罩之下表单照样 visible，
  所以它一直通过。改为直接 `fill()` 一次，可操作性检查会因遮挡失败。
- 修正二（2026-09-10，用户反馈）：把弹窗改成可关闭时，我**同时把路由拦截也去掉了**，
  于是关掉弹窗后全站畅通——用户能进去，但页面上的动作在后端一律失败，看到的是
  「能进去，但什么都做不成」。
  改为**判定、呈现、拦截三分**：`requiresModelSetup` 判定，`ModelSetupDialog` 呈现
  （可关闭），`blockedByMissingModel` 拦截（与弹窗是否被关掉无关）。配置页永远放行，
  平台管理员优先送去运维页——配平台默认模型能让所有人都能用。

### D-033：复核表的行内「确认」只给可采纳行，不照搬设计稿的每行一致

- 状态：`Accepted`；2026-09-10（设计稿对照）。
- 背景：设计稿「待确认给分」表每一行末尾都是「查看原文 / 确认」两个动作，行与行长得
  一样。实际队列里混着三类行：普通低置信度项（有 AI 分，可采纳）、无 AI 分的 blocked
  项、以及 `ManualReviewTask` 阻塞任务。
- 选择：**「查看原文」每行都给；「确认」只给 `acceptable` 的行**。阻塞行与无分行不渲染
  确认按钮，`acceptOne` 对它们直接返回 `null`，不发请求。
- 原因：阻塞任务缺的是结论本身——provider 调用失败、规则执行被阻断，采纳一个不存在的
  分数没有意义；无 AI 分的项同理。给它们一个必然 400 的按钮，是把用户送去点一个注定
  失败的动作，而失败信息还解释不了原因。
- 放弃：
  - **每行一致地给「确认」，点了再报错**（设计稿的字面形态）。放弃理由如上。
  - **给阻塞行一个置灰的「确认」**。置灰能表达「现在不能点」，但表达不了「这一项要走
    领取—凭证据解决的另一条流程」，反而暗示等一等就能点。阻塞任务的处理入口是
    V3-6（claim/release/resolve），尚未实现；在它落地之前，不摆一个假的入口。
- 代价/约束：界面上同一张表的行动作不整齐。这是有意的——不整齐正是在告诉用户这几行
  不是一类东西。V3-6 落地后，阻塞行应补上「领取」而不是「确认」。

### D-034：逐项确认与批量采纳共用一条提交路径

- 状态：`Accepted`；2026-09-10。
- 背景：新增行内「确认」时，最直接的写法是再写一个只提交一条的函数。
- 选择：抽出 `accept(items, reason)`，`acceptVisible` 与 `acceptOne` 都调它。
- 原因：两者的服务端前置条件**完全一致**——`result_revision` 前置、每项 `review_revision`
  乐观并发、幂等键每次新生成、409 要显式提示刷新。这四条里漏任何一条都不会立刻报错，
  而是在并发时静默覆盖别人的复核结果。两份实现意味着两个地方要各自记得这四条。
- 放弃：为逐项单独写一个精简版本（少带 `result_revision`，因为「只有一条，冲突概率低」）。
  概率低不等于不发生，而发生时的后果是复核结论被悄悄覆盖。
- 代价/约束：`acceptOne` 的入参是队列条目对象而不是 id，调用方必须持有那一行的
  `review_revision`。

### D-035：AI 起草的落库走 `recompile`，前端做合并

- 状态：`Accepted`；2026-09-10（设计稿对照发现 V3-2 断链）。
- 背景：起草端点按设计 non-persistent。前端拿到建议后没有任何界面呈现它，也没有把它
  写回去的路径——`生成全部缺失细则` 点完，阻断项一个没少，发布仍然被挡。
  「模板标准的完整生成到使用流程」在生产上走不通。
- 选择：确认在前端完成（逐行排除），合并后经 `POST /rubrics/{id}/recompile` 落库；
  合并逻辑抽成纯函数 `lib/ai-draft.js`。
- 原因：
  - `recompile` 是现成的、唯一可用的写入口。`PATCH /rubrics/{id}` 遇到已有编译产物
    直接 409，而工作台里的标准在导入时就有编译产物。
  - 不加新端点就不用加迁移，也不动已经通过门禁的后端合同。
  - 合并是纯数据变换，抽出来能单测；留在组件里只能靠挂载整页去验。
- 放弃：
  - **加一个 `POST /rubrics/{id}/draft-deduction-rules/apply` 服务端端点**，由后端持有
    未确认建议。那需要一张新表存中间态，连带 `pgs_app` 授权与 RLS，以及「建议什么时候
    过期」这一整套生命周期——而这些建议的生命周期本来就只有一次会话那么长。
  - **起草时直接落库、事后再撤**。已经写进执行草稿的规则会立刻影响校验与发布判断，
    「先生效再撤销」比「先确认再生效」危险得多。
  - **逐条打勾确认**（设计稿是每行一个「确认」）。一次生成十几条规则是常态，多数时候
    用户全都要；反过来要求逐条打勾，把常见路径变成十几次点击。改为默认全要、**逐条
    排除**，并保留撤销。
- 代价/约束：
  - 每次应用都生成一份新的执行草稿（版本重名时服务端自加 `-draft.N`）。草稿会累积，
    发布时仍需用户显式选定哪一份。
  - 建议只活在这一次会话里，刷新即失。重新生成要再花一次真实调用。
  - 起草成功后**不再刷新完整度**——刷了也不会变，只会把刚拿到的建议换回一模一样的
    旧数字，看起来像什么都没发生。

### D-036：删掉 `StageStub` 与指向旧版界面的文案

- 状态：`Accepted`；2026-09-10。
- 背景：旧 SPA 已于 2026-09-08 下线（D-024），`/legacy/` 返回 404。但评分标准页仍写着
  「导入、AI 起草与逐条审核仍在**旧版模板中心**完成」，其中一处是 `<a href="/">`
  ——点下去回到工作台首页；空列表提示也写着「请先在旧版模板中心导入」。而这两件事
  **本页就能做**。`components/StageStub.vue` 是并存期的占位组件，已无人引用，模板里
  还挂着一个指向旧版工作台的链接。
- 选择：改写两处文案，删除 `StageStub.vue`，并加源码契约 `retired-entry.test.js` 挡住
  回流。
- 原因：这类文案不会让任何测试变红——链接合法、页面渲染正常，它只是把人送去一个不
  存在的地方。只能靠契约把它变成能自动发现的。
- 放弃：
  - **保留 `StageStub` 以备将来复用**。它绑的是「新旧并存」这个已经结束的阶段，留着
    只会让下一个人以为并存期还在。
  - **契约扫描整份 `.vue`**。那会把 `<script>` 里解释历史的注释（「原先散落在旧 SPA
    三处」）一起禁掉，逼人删掉唯一记着来龙去脉的那句话。改为只扫 `<template>`。
- 代价/约束：契约按中文短语匹配，换个说法（比如「老版本页面」）绕得过去。它挡的是
  复制粘贴式的回流，不是刻意绕开。

### D-037：篇章与格式发现接进工作区，复用已到手的 run 对象

- 状态：`Accepted`；2026-09-10（设计稿对照）。
- 背景：两组发现一直被持久化、导出、并参与扣分，但工作区从不显示。复核的人看到某项被
  扣了分，却查不到依据，只能另外下载 HTML 报告。
- 选择：中栏改为三页签；发现项取自 `selectPaper` 已经拿到的 run 对象，不发新请求。
- 原因：`ScoringRunRead` 本来就带这两个字段，store 此前只从中取了 `id`。再加一个
  `GET /scoring-runs/{id}` 等于为同一份已经在内存里的数据多跑一趟。
- 放弃：
  - **新增一个 findings 专用端点**。没有任何新数据要取，只会多一条要维护的合同。
  - **把发现项混进右栏评分表**。它们是按 run 产生的、跨评分项的观察；挂到某一项下面会
    暗示只影响那一项，而 `deducted_by` 恰恰说明有的影响了、有的没有。
  - **无发现时给空表**。空表说明不了「查过没问题」还是「压根没查」，而后者在非 docx
    提交上是常态。
- 代价/约束：`deducted_by` 为空显示成「未计入」是一句断言——它依赖 findings→扣分那条
  链路如实回填。若将来有新的消费路径忘了标 `deducted_by`，界面会说错话。

### D-038：`.btn` 的居中修在类本身；文件框改样式而不换控件

- 状态：`Accepted`；2026-09-10（生产走查，同一根因第三次出现）。
- 背景：`.btn` 没设 `display`，用在 `<a>` 上文字偏上 8px。前两次分别在
  `ModelSetupDialog` 与 `ReviewView` 里补了局部规则，第三次又在「新建评分任务」上出现。
- 选择：把 `display: inline-flex` 等四行写进 `.btn`，删掉两处局部补丁；验收契约改为
  **按几何量**（`Range` 取文字矩形与元素矩形比中心，容差 1px），并扫全部页面的 `a.btn`。
- 原因：局部补丁只覆盖写补丁的那一处，而这个类在十几个地方被用。按计算样式断言
  （`display` 含 flex）也不够——它锁死了实现方式，却证明不了文字真的居中。
- 放弃：
  - **继续按视图打补丁**。已经证明不收敛。
  - **验收断言 `getComputedStyle` 的 display**（原弹窗用例的写法）。换一种居中写法
    （比如 `line-height: 36px`）它会误报失败，而文字偏了它未必报。
- 代价/约束：`.btn` 变成 `inline-flex` 后不再参与文本基线对齐。目前没有把 `.btn` 混排
  在正文句子里的用法；如果将来有，那一处要单独处理。

文件选择框：

- 选择：`::file-selector-button` 直接改按钮外观，输入框本体保持原生。
- 放弃：**藏掉 `<input type="file">`，用 `<label class="btn">` 冒充按钮**。那是常见做法，
  但会丢掉原生的键盘可达性与「已选文件名」的无障碍播报，得另外补一整套焦点环与
  `aria-describedby`。为一个纯外观问题引入那些，代价不对等。
- 代价/约束：「未选择任何文件」是浏览器画的，**改不掉文案**，也无法本地化成别的说法；
  只有字号与颜色跟着输入框走。各浏览器的这段文字不一致。

### D-039：弹窗按「这一次导航被拦了」显示，不按会话级开关

- 状态：`Accepted`；2026-09-10（用户报缺陷）。
- 背景：`ModelSetupDialog` 用组件级的 `dismissed` ref 记「已经关过了」。组件挂在
  `AppShell` 上跨路由存活，于是**关掉一次整个会话不再提示**。拦截判据一直是实时的
  `can_use_llm`，这没错；错的是呈现层被一次性标记锁住，用户点「评分任务」只会被静默
  弹回配置页，屏幕上没有任何解释。
- 选择：守卫拦截时 `noteBlockedAttempt(to)` 写入一条新记录；弹窗的显示条件是
  `requiresModelSetup && pendingBlock !== null`；关闭调 `clearBlockedAttempt()`，只清
  当前这一条。**没有任何标记跨越两次导航。**
- 原因：
  - 「可关闭」与「每次都提示」并不矛盾——冲突只在于把关闭的作用域定成了会话。改成
    「作用于这一次拦截」，两个诉求同时成立：在配置页上关掉弹窗能填表（2026-09-10 的
    修复保留），离开后再点任何入口又会重新提示。
  - 弹窗顺带说出用户**本来想去哪一页**。只说「没有模型」，他仍然不知道自己刚点的
    那一下发生了什么。
  - 跳转带锚点（`#ai-connections` / `#platform-llm`）并高亮目标区块：那两页都还有别的
    内容，落在页顶用户得自己找。
- 放弃：
  - **完全不可关闭**。已经试过（D-032 初版），遮罩会盖住配置表单，用户到了目的地反而
    填不了表。
  - **把「关过了」存进 `sessionStorage`**。那是同一个错误的更持久版本——刷新都清不掉。
  - **在每个页面组件里自查**。要在七个页面重复，漏一个就是一个洞。
  - **放到 axios/fetch 拦截器**。太晚：页面已经渲染，用户可能填了半天表单才被拦；而
    `can_use_llm` 是会话级状态，不是单个请求的属性。
- 代价/约束：
  - `pendingBlock` 是模块级 ref，跨组件共享。单测必须在 `beforeEach` 里
    `clearBlockedAttempt()`，否则用例之间互相污染。
  - 守卫重定向到当前已在的路由时，Vue Router 会判为重复导航并中止——但 `noteBlockedAttempt`
    在返回重定向之前已经执行，弹窗照常出现。这一点是有意依赖的顺序。

### D-040：能力表在任何改变可用性的写操作后立即刷新，双向

- 状态：`Accepted`；2026-09-10（同一轮缺陷）。
- 背景：`OpsView` 保存平台模型后调了 `session.loadCapabilities()`，`AccountView` 绑定
  BYOK 连接后**没调**。普通用户配好自己的连接，拦截不会解除，得手动刷新页面——而界面上
  没有任何东西提示他要这么做。
- 选择：新建连接后刷新；**停用与删除之后也刷新**。
- 原因：反向的洞更隐蔽。停用最后一个连接后 `can_use_llm` 实际已变 `false`，前端仍以为
  可用，于是放人进功能页，然后每个动作在后端失败——「能进去，但什么都做不成」，正是
  D-032 修正二治过的那个病。
- 放弃：
  - **轮询能力表**。可用性只在这几个写操作后变化，轮询是拿持续开销换一个本来就知道
    时机的事件。
  - **本地推断**（前端自己把 `has_own_connection` 置 true）。判据在后端（`status=active`
    且未删除，还要叠加平台模型），前端复制一份迟早与后端分叉。
  - **换 Key（rotate）后也刷新**。轮换不改 `status`，可用性不变，刷了是白跑一趟。
- 代价/约束：每次连接增删启停多一次 `/system/capabilities` 请求。这些都是低频的显式
  操作，不是热路径。

### D-037：导入条款逐条确认与当前评分项批量确认

- 状态：Accepted；2026-09-11，用户批准实施。
- 背景：工作台仅展示导入条款的待确认摘要，AI 临时建议的确认面板不能处理已导入规则。
- 选择：逐条确认即时持久化；批量只处理当前评分项，保留已确认条款，遇失败立即停止并报告已完成数量。确认不自动发布。
- 采用独立审核正文投影，不扩大 `execution-draft` 的正文暴露范围；确认校验用户实际看到的规则身份与内容，避免旧页面确认新版本。
- 放弃的替代方案：只补按钮但不提供正文核对、以重新编译代替原子规则审核，均不采用；前者无法核对，后者会重建规则身份并丢失既有审核进度。
- 对 D-035 的调整：保留 AI 建议经 recompile 应用的约束；撤销其“无逐条确认”的界面选择，按本次原型要求提供单条及当前评分项批量动作。
- 代价：批量不是跨条款原子事务，必须显示部分完成；AI 建议与已导入规则的生命周期继续分别处理。AI 分次追加时，只复用生成指纹、行身份、条款内容及评分上下文完全相同的既有确认，并在新编译事务内记录引用前序规则的审核事件；内容或上下文改变仍需重新确认。
- 用户明确允许本次新增审核 API 不新增无数据库结构变化的 Alembic 迁移；必须保留现有迁移验证。
- 补充决定（Accepted）：用户选择本次补齐完整原子规则编辑，不采用只禁用编辑并要求重新导入的临时方案。新增完整规则编辑输入，保留每条规则及来源，以新草稿承载修改；内容摘要在父行锁内复核。AI 补全只追加带生成指纹的建议。修改总分时同步全局政策总分及 hash，不自动重设其它评分政策。

### 2026-09-14 OpenRouter 规则起草错误处理（Accepted）
...
生产日志确认上游 HTTP 200 后应用返回 503；历史日志未保留安全解析原因，不能追认具体原因。Chrome 本次复现为 422，响应码 MUTEX_GROUP_MISSING，且客户端忽略 detail.message/user_action。选择补齐必填字段指令、一次有限纠正、稳定解析错误分类和安全原因日志，并按评分项请求以保留部分结果。拒绝放宽互斥/分值/来源校验、静默填充评分规则、无限重试和记录原始模型正文。代价是无效输出可能多调用一次模型；不保证免费路由永远返回合格输出，连接/额度/输出预算问题仍需明确报错。未确认建议保留在页面，刷新会丢失，人工确认边界不变。无数据库结构变更。

### 2026-09-15 AI 规则组上限与单条上限冲突（Accepted，2026-09-15 用户授权实施及修复生产草稿）

背景：生产 15 条 AI 扣分规则同时为 once 和 cap_points=20，源于组字段进入单条字段，审核保存不等于发布合同通过。

实施：前后端统一组上限与单条上限的含义。AI 单次互斥规则编译为 once、单条 cap_points=null、levels=[]；原组上限保留可追溯元数据，并验证组身份、单次模式及每档分值不超过组上限。仅满足这些条件时，互斥最多一档已保证组上限，无需改 Core 计分。任意人工/Excel cap、缺失来源或无法证明等价的组合保持阻断，不能全局清空。后端作为最终边界覆盖旧前端请求。编辑器按重复策略限制字段；校验反馈具体规则和中文原因。

现有草稿通过显式原子编辑差异与现有重新编译流程生成新草稿，保留原始来源和旧版本；按现有内容指纹规则重新审核受影响条款，不篡改旧确认记录或自动发布。修复后不保证其它独立阻断一并消失。

不采用放宽发布校验或把 once 改成 capped：前者掩盖模型不一致，后者可能把一次扣分变成多次累计。代价是补充映射与修复回归、部分条款重新确认；不新增数据库表字段，若实施发现必须扩展组计分语义，应另立方案与迁移、QWK 验证，不隐式改变评分政策。

### 2026-09-15 发布流程按人工步骤呈现（Accepted）

背景：条款全部确认且校验通过后，草稿仍显示禁用发布按钮和过时的未确认提示，而提交审核在页头，用户无法在当前卡片理解下一步。用户要求依照人工操作顺序修改，并保持现有设计规范。

选择在第 3 步集中处理：先检查和完成待办，再提交审核，最后核对版本/范围后显式发布。只有对应阶段显示发布设置和发布动作；完成检查时提示下一步提交审核；提交失败停留原阶段且显示错误；已发布字段禁用并提示复制新版本。保留独立审核与发布事务，不采用“一键自动审核并发布”或放宽状态守卫，以免用户在核对范围前冻结版本。代价是保留两个明确的业务操作，但入口集中、反馈随状态更新；沿用既有卡片/表单/按钮/状态标签，不增加自定义设计体系。

## 4. 维护记录

| 日期 | 主题 | 决策处理 |
|---|---|---|
| 2026-09-13 | 评分标准条款确认与原型还原 | 落实用户补充选择：完整原子规则编辑在本次交付，不以评分项重新编译替代；保留来源和独立规则，确认与发布分离。普通评分项与原子编辑均同步用户修改的政策总分及 hash，不改变其它政策。95d189f 已按完整 CI 门禁部署，生产登录后的流程不以 CI 冒烟代替验收。 |
| 2026-09-11 | 评分标准条款确认与原型还原 | 接受 D-037，修订 D-035 的逐条确认取舍；确认即时保存、批量限定当前评分项、失败停止且不自动发布。保留原始起草上下文并升级起草/缓存版本，代价是旧缓存失效。分值不一致须用户在基本信息页显式修正，不自动重分配。 |
| 2026-09-10 | 未配置模型的拦截修复 | 接受 D-039（一次性标记改为按次拦截记录）与 D-040（能力表双向刷新）。修正 D-032 的呈现层缺陷；判定与拦截层未改。 |
| 2026-09-10 | 按钮居中与文件选择框 | 接受 D-038：根因修在类上、验收改为按几何量；文件框保留原生控件。未改后端。 |
| 2026-09-10 | 复核进度条 | 复用 D-037 同一条原则：0 与「没有」不是一回事，空态走单独分支而不是画一条 0% 的条。无新 ADR。 |
| 2026-09-10 | 工作区中栏三页签 | 接受 D-037：发现项接进界面，不新增端点。未改评分语义。 |
| 2026-09-10 | AI 起草闭环与旧入口清理 | 接受 D-035（起草确认经 `recompile` 落库）与 D-036（清掉已下线入口文案并加契约）。未加端点、未加迁移、未改评分语义。 |
| 2026-09-10 | 复核表行内动作 | 接受 D-033（确认只给可采纳行，阻塞行不摆假入口）与 D-034（逐项与批量共用提交路径）。未改评分语义与后端合同。 |
...
| 2026-09-10 | 拦截与弹窗分离 | 修正二：关掉弹窗不再等于放行；拦截回到路由守卫，弹窗只负责解释。 |
| 2026-09-10 | 弹窗可关闭 | 修正 D-032：阻断式改为可关闭；补记「验收断言可见而非可点击」这个假绿灯。 |
| 2026-09-09 | 模型未配置的呈现方式 | 接受 D-032：阻断式弹窗替代跳转加横幅。未改判定逻辑与后端。 |
| 2026-09-09 | V3-3 发布时选分享范围 | 实施 D-029：`publish` 接受 `visibility`，与编译产物同事务生效；范围须先于发布写入（ORM 守卫拦「已发布不可改」）。跨组织维持 platform_admin。 |
...
| 2026-09-11 | 平台模型配置默认折叠 | D-028 展示补充（Accepted）：已配置表单持续占用页面空间，采用默认收起并保留展开入口与独立测试按钮；保存和停用随表单收起。不采用永久隐藏，以保留修改入口；折叠状态仅保留在当前页面，API Key 仍需重新输入且保存后清空。 |
...
背景：生产直传仅归档，队列误将 uploaded 标为 done，缺少解析调用。选择前端逐文件串行衔接现有解析端点，检查响应业务状态并保留已归档 ID；不采用重新上传或放宽预检，以避免重复材料及未解析评分。代价：处理时间包括解析；单文件失败隔离，重试后重新预检。
...
背景：平台运行时 platform 标识被当作用户连接外键，导致生产用量落库失败及评分中残留。决定保留 ScoringRun 中的平台用量与来源，仅真实 BYOK 写用户连接外键/账本；不创建伪造平台用户连接，不放宽外键或静默忽略任意账本错误。异常批次使用已有 scored_with_errors 状态，先回滚失败事务再落库；拒绝重复启动不影响正在执行的任务。代价：平台用量不进入面向用户自有连接的账本，需从评分记录查询；本次不引入平台独立账本或后台执行器。
...
### 2026-09-15 持久化后台评分（Accepted）

背景：生产一批三份材料累计 42 次成功模型调用，整批同步请求在 Vercel 300 秒上限处被终止。选择复用现有数据库持久化批任务，由短 API 创建任务，并通过 Vercel Queues Python subscriber 按材料执行；数据库按材料提交结果并用租约、心跳和幂等键恢复。任务列表把“评分中”及其进度作为明确的运行详情入口，并提供“正在评分”聚合页；详情页承载进度、心跳、逐材料状态和有限操作。普通评分使用服务端 fail-closed 观察策略默认值，用户无需填写 Core 发布门禁参数；缺少 paired comparison 时仍不得授权 Core 切换。

拒绝继续提高 HTTP 超时、由浏览器保持长连接、用 Vercel Cron 轮询整批、由浏览器串行触发多次请求，以及引入 Render 常驻 worker。也暂不采用 Workflow：现有数据库已经实现业务状态、取消、重试与页面查询，Workflow 会形成第二套执行状态并扩大改造面。队列消息只含内部 ID，默认并发 2、重试间隔 30 秒、最多投递 12 次；每份材料仍需在单个 Vercel 300 秒函数期限内完成，若生产证据显示单份也超限，再把评分项拆为更细步骤。收益是关闭浏览器或发布新版本不影响已接收消息，且无需第二个部署平台；代价是依赖 Vercel Queues beta 和按操作/Fluid compute 计量。数据库已有 job/item 表，按用户授权不新增无实际结构变化的 Alembic 迁移。

生产回归发现兼容模型服务对 32k、16k token 预算内的序列化请求仍返回 HTTP 413，并且 Core 阻断结果被队列误记为材料成功。决定把默认上下文预算收紧为 8k，并在队列成功检查点增加“全部评分项均有有效分数”的业务门槛；不完整运行保留审计但不得作为可复用成功结果。拒绝把空分数解释为 0 分或仅靠页面隐藏异常，因为这会污染复核和用量判断。

维护记录：2026-09-15 · 持久化后台评分：用户授权按方案开发并部署，决策转为 Accepted。

维护记录：2026-09-15 · Vercel-only 后台评分：用户明确取消 Render 部署，接受范围调整为同一 Vercel 项目的 Queues Python subscriber；保留数据库为业务状态来源。
