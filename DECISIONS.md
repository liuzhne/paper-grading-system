...
背景：生产一批三份材料累计 42 次成功模型调用，整批同步请求在 Vercel 300 秒上限处被终止。选择复用现有数据库持久化批任务，由 Vercel 快速创建任务、常驻容器 worker 执行，按材料提交结果并用租约和心跳恢复。任务列表把“评分中”及其进度作为明确的运行详情入口，并提供“正在评分”聚合页；详情页承载进度、心跳、逐材料状态和有限操作。普通评分使用服务端 fail-closed 观察策略默认值，用户无需填写 Core 发布门禁参数；缺少 paired comparison 时仍不得授权 Core 切换。

拒绝继续提高 HTTP 超时、由浏览器保持长连接、由 Vercel Cron 执行整批评分、一次请求拆成多次但仍由浏览器串行触发，以及把 HTTP 200 当作评分完成。初始材料并发为 2，并依据 provider 配额与耗时观测调整。常驻执行器采用 Render Docker background worker 的最小 `0.5c-512mb` 规格，地区为 Singapore，自动发布只在 GitHub checks 通过后触发；密钥通过 Render 环境变量注入。代价是增加常驻 worker、固定月费与调度运维；收益是刷新或断网不影响执行，平台终止后能按租约从检查点恢复。数据库已有 job/item 表，按用户授权不新增无实际结构变化的 Alembic 迁移。

维护记录：2026-09-15 · 持久化后台评分：用户授权按方案开发并部署，决策转为 Accepted；实现采用独立容器 worker，Vercel 仅创建和查询任务。
