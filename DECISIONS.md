...
生产回归发现兼容模型服务对约 32k token 预算内的序列化请求仍返回 HTTP 413，并且 Core 阻断结果被队列误记为材料成功。决定把默认上下文预算收紧为 16k，并在队列成功检查点增加“全部评分项均有有效分数”的业务门槛；不完整运行保留审计但不得作为可复用成功结果。拒绝把空分数解释为 0 分或仅靠页面隐藏异常，因为这会污染复核和用量判断。
