...
- Export: openpyxl
- Current verification: Alembic head `0028_export_event_backfill`; Python 3.12 lock run（Python 3.10+ supported；准确用例数以当前 CI 为准）
...
- **每次部署先迁移**：`uv run alembic upgrade head`（当前到 `0028_export_event_backfill`；测试用 `create_all`，生产必须走迁移；Docker app 容器启动时会自动迁移）。0022 会把升级前的单租户资源回填到默认组织，并把旧默认开发用户提升为 Bootstrap Admin；0023 新增规则检查点与人工复核队列；0024–0028 补齐批次状态机、结构化复核原因、命令幂等回执与导出事件，**含数据时一律拒绝有损降级**；迁移不会复制任何 `.env` LLM Key。
