...
- Export: openpyxl
- Current verification: Alembic head `0023_rule_scoring_review_tasks`; Python 3.12 lock run（Python 3.10+ supported；准确用例数以当前 CI 为准）
...
- **每次部署先迁移**：`uv run alembic upgrade head`（当前到 `0023_rule_scoring_review_tasks`；测试用 `create_all`，生产必须走迁移；Docker app 容器启动时会自动迁移）。0022 会把升级前的单租户资源回填到默认组织，并把旧默认开发用户提升为 Bootstrap Admin；0023 新增规则检查点与人工复核队列，含审计数据时拒绝有损降级；迁移不会复制任何 `.env` LLM Key。
