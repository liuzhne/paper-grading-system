...
不自动调整等级阈值、舍入或评分算法。AI 指纹建议走追加合并，不能替换导入规则。
...
| 日期 | 主题 | 架构核对结果 |
|---|---|---|
| 2026-09-13 | 评分标准条款确认与原型还原 | 完成原子编辑、完整来源复制、AI 追加、锁内内容摘要复核及结构阻断展示；本地 1911 项后端与 139 项浏览器回归通过。数据库与默认 legacy 不变，待 CI 发布。 |
