...
毕业论文智能评分系统：评分标准(Excel+Word批注) → 批次 → 上传论文(解析) → 证据/确定性检查 → 逐项评分 → 篇章一致性/格式核验 → 人工复核 → 报告/导出。后端 FastAPI + SQLAlchemy + Alembic；默认 Mock LLM，配 env 切真实模型。
...
## 架构（backend/app/）
- `api/routes/`：rubrics / batches / papers / scoring / exports / system / calibration。
- `services/`：
...
## 约定
- 新增端点/字段要配 Alembic 迁移（已到 0007）+ 对应测试（`backend/app/tests/test_*.py`，复用 `conftest` 的 `client` 与 `make_*` 造数据）。
