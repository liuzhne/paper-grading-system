...
def _login(client, monkeypatch, username):
    ...
    with client.session_factory() as session:
        ...
        token = session.scalar(select(models.EmailVerificationToken).where(models.EmailVerificationToken.user_id == user.id))
    assert client.post("/api/auth/verify-email", json={"token": token.token}).status_code == 204
